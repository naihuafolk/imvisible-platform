(function (RP) {
  ...
  RP.views.m2 = function () {
    ...
    // KPI: ตัวเลขชุดนี้คำนวณจากคิวตัวอย่าง — บัญชีจริงต้องไม่เห็น
    var kpiBlock = RP.isReal()
      ? ui.card({
          title: 'สรุปคิวผลิตคอนเทนต์',
          sub: 'ตัวเลขจะขึ้นเมื่อระบบผลิตบทความจริงในบัญชีของคุณ',
          cls: 'mb',
          body: RP.noData('ยังไม่มีข้อมูล',
            'บัญชีนี้ยังไม่มีบทความที่ระบบผลิตจริง — กด “โหลดโปรเจ็ค & บทความจริง” ด้านบน แล้วกด “ผลิตเดี๋ยวนี้” ' +
            'ระบบจะเขียน ตรวจข้อเท็จจริง และตรวจความซ้ำ แล้วตัวเลขจริงจะขึ้นที่นี่ (ปกติใช้เวลาไม่กี่นาทีต่อบทความ)')
        })
    ...
    var html =
      ...
      // แถบกำกับสถานะข้อมูล (ตัวอย่าง vs บัญชีจริงที่ยังไม่มีบทความ)
      RP.sampleNotice('คิวคอนเทนต์และตัวเลขสรุปในหน้านี้') +
      RP.collectingNotice('จากบทความที่ระบบผลิตให้บัญชีของคุณ') +
      liveGenCard() +
      liveRealCard() +
      kpiBlock +
      ui.card({
        ...
        body: RP.realOr(queueTable(queue), {
          title: 'ยังไม่มีบทความในคิว',
          hint: 'เราไม่แสดงบทความสมมติ ผลตรวจ Fact-Check หรือ % ความซ้ำที่ไม่ได้ตรวจจริง — ' +
            'รายการจะขึ้นเมื่อระบบผลิตบทความให้บัญชีของคุณจริง (ดูรายการจริงได้ที่การ์ด Live ด้านบน)'
        }),
        flush: !RP.isReal()
      }) +
