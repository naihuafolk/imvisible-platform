...
7. โครงสร้าง — เริ่ม H2, H3 ซ้อนใต้ H2 เท่านั้น ห้ามข้ามชั้น ห้าม H1 (ธีมใส่เอง) ย่อหน้าสั้น 2-4 บรรทัด ใช้ <ul>/<ol> เมื่อมีขั้นตอน ใช้ <table> (≤4 คอลัมน์) เมื่อเปรียบเทียบ/ราคา · ทำให้สแกนบนมือถือง่าย: เน้นคำ/ตัวเลข/ข้อสรุปสำคัญด้วย <strong> · แทรก callout สั้นด้วยรูปแบบ <p><strong>เคล็ดลับ:</strong> ...</p> / <p><strong>ข้อควรระวัง:</strong> ...</p> / <p><strong>สรุปเร็ว:</strong> ...</p> (ป้ายนำหน้าด้วย <strong>) — ห้ามใช้แท็กนอกรายการเพื่อทำกล่อง
8. KEYWORD ธรรมชาติ — primary ใน H2 แรก + 100 คำแรก + title; secondary/LSI กระจาย ไม่ stuffing (≤3%)
9. ภาษาไทยเจ้าของภาษา — เขียนเหมือนผู้เชี่ยวชาญคนไทยเล่าให้เพื่อนฟัง ไม่ใช่แปลจากอังกฤษ · สลับประโยคสั้น (6-12 คำ) กับยาว อย่าให้ทุกประโยคยาวเท่ากัน · อ่านออกเสียงต้องลื่น · หนึ่งประโยคหนึ่งใจความ · ใช้กริยาแทนการแปลงเป็นคำนาม ('จอง' ไม่ใช่ 'ทำการจอง', 'ช่วย' ไม่ใช่ 'ให้ความช่วยเหลือ', 'เลือก' ไม่ใช่ 'ทำการเลือก') · เลี่ยง passive 'ถูก...' ถ้าไม่จำเป็น · ห้ามกลิ่นแปลเครื่อง/สำนวนแข็ง: 'มันเป็นสิ่งที่','ในการที่จะ','การที่','ซึ่งเป็น','อันเป็น','มีความสำคัญเป็นอย่างยิ่ง','เป็นอย่างมาก','อย่างมีประสิทธิภาพ','ให้ความสำคัญกับ','ทำการ+กริยา' · ห้ามคำเชื่อม/วลี AI ซ้ำซาก: 'อย่างไรก็ตาม','อย่างไรก็ดี','โดยสรุปแล้ว','ในยุคดิจิทัล','ในโลกปัจจุบัน','เป็นที่ทราบกันดีว่า','จะเห็นได้ว่า','ด้วยเหตุนี้','นอกจากนี้','อีกทั้ง','ที่สำคัญไปกว่านั้น' — ถ้าต้องเชื่อมให้ใช้คำที่คนไทยพูดจริง ('แต่','พอ...ก็') หรือขึ้นประโยคใหม่ · ตัวเลข: เลขอารบิก (25 ไม่ใช่ ๒๕) มีตัวคั่นหลักพัน (1,500) หน่วยเงิน 'บาท' หลังเลข ช่วงใช้ '–' (3,000–5,000 บาท) · สรรพนามผู้อ่าน 'คุณ' สม่ำเสมอ · YMYL (การเงิน/สุขภาพ/กฎหมาย) ต้องน่าเชื่อถือ + แหล่งอ้างอิง + disclaimer
...
async def _anthropic_chat(system: str, user: str, max_tokens: int = 16000, model: str | None = None) -> str:
    """เรียก Claude · ไม่มี temperature/thinking/prefill → เข้ากับ Fable 5 ได้
    ถ้าเป็น Fable/Mythos (พรีเมียม) เปิด server-side fallback ไป Opus 4.8: ปฏิเสธ/ใช้ไม่ได้ = ตกไป Opus ในคอลเดียว บทความไม่ล่ม"""
    ...
    if ("fable" in model or "mythos" in model):
        fb = settings.anthropic_model or "claude-opus-4-8"
        if fb != model:
            headers["anthropic-beta"] = "server-side-fallback-2026-06-01"
            payload["fallbacks"] = [{"model": fb}]
    # Fable ช้ากว่า (คิดลึก) → เผื่อเวลาอ่านนานขึ้น
    async with httpx.AsyncClient(timeout=httpx.Timeout(900.0, connect=15.0)) as c:
        r = await c.post("https://api.anthropic.com/v1/messages", headers=headers, json=payload)
        r.raise_for_status()
        data = r.json()
    if data.get("stop_reason") == "refusal":       # ทั้ง chain ปฏิเสธ → คืนว่างให้ _llm ลองเจ้าอื่น
        return ""
...
async def _llm(system: str, user: str, tier: str = "fast") -> tuple[str, str]:
    ...
    for prov in order:
        ...
        try:
            if prov == "anthropic":
                # สเตจงานเขียน (premium/strong) → ใช้โมเดล 'เขียนดีสุด' (Fable 5); tier อื่น (fast/ลินต์) → Opus (เร็ว/ถูกกว่า)
                mdl = (settings.anthropic_write_model or settings.anthropic_model) \
                    if tier in ("premium", "strong") else settings.anthropic_model
...
ทำแล้วส่ง JSON คีย์: primary_intent (info|commercial|transactional|navigational + เหตุผล 1 ประโยค), thai_persona {who, real_worry, decision_factor}, target_keywords {primary, secondary[3-6], entities[>=10]}, entity_glossary (คำเรียก entity หลักที่ใช้เหมือนกันตลอด), key_takeaways[3-5] (แต่ละข้อ statement สั้น self-contained มีตัวเลข/ข้อเท็จจริง 1 อย่าง ก็อปไปตอบได้; ไม่รู้จริงใส่ [ต้องเติม] ห้ามแต่ง), definitions[2-4] {term, def ('term คือ ...' 1 ประโยคสมบูรณ์)}, content_gaps[3-5] (สิ่งที่คู่แข่งยังขาด เจาะจง), information_gain_angles[3-5] (มุม/ข้อมูล/การเปรียบเทียบ/ตัวอย่างที่ยังไม่มีใน SERP ปัจจุบัน เจาะจงตรวจสอบได้; ไม่รู้จริงใส่ [ต้องเติม]), outline[] แต่ละ H2 = {h2_text (ถ้าเป็นคำถามใช้ภาษาคนไทยพิมพ์จริง), intent_covered, answer_seed (คำตอบตรง 40-60 คำ statement ขึ้นต้นด้วย subject/entity), must_include_facts (ตัวเลข/ปี/ราคา — ไม่รู้จริงใส่ [ต้องเติม: ...] ห้ามแต่ง), format (paragraph|table_price|table_compare|numbered_steps|checklist), covers (คำถาม/gap ที่รับผิดชอบ), cta (none|soft), h3_subpoints[]}, faq[4-8] {q, answer_seed}, suggested_meta {title_tag<=60, meta_description 150-160, url_slug}, internal_link_anchors[3-5], word_count_target
สำคัญ: ต้อง assign ทุก content_gap และทุกคำถามข้างบนลง H2/H3 อย่างน้อย 1 จุด รวม H2 ปิดท้าย 'คำถามที่พบบ่อย' · ต้องมีอย่างน้อย 1 H2 ที่ format=table_compare หรือ table_price (ตารางเทียบ/เกณฑ์ตัดสินใจ/ช่วงราคา) และ 1 H2 'ข้อผิดพลาด/ข้อควรระวังที่พบบ่อย' เพื่อสร้าง information gain เหนือคู่แข่ง · ส่งเฉพาะ JSON"""
...
กฎ (ห้ามละเมิด):
- เปิดด้วย <p> answer-first "40-55 คำเป๊ะ" (ช่วงที่ Google หยิบเป็น Featured Snippet และ AI หยิบไปตอบง่ายสุด) ตอบคำถามหลักตรง ๆ ขึ้นต้นด้วย subject/entity เน้นวลีสำคัญด้วย <strong>; ถ้าหัวข้อเป็น how-to/เปรียบเทียบ/คู่มือยาว ตามด้วย <ul> 'ประเด็นสำคัญ' 3-5 ข้อ แต่ละข้อเป็นข้อเท็จจริงกระชับ quotable (ตัวเลข/เกณฑ์/ข้อสรุป) แล้วจึงขึ้น <h2> ตัวแรก
...
- ย่อหน้า 2-4 บรรทัด · เขียนภาษาไทยแบบเจ้าของภาษา (ไม่ใช่แปลจากอังกฤษ) สลับประโยคสั้น-ยาว ใช้กริยาแทนคำนามธรรม ('จอง' ไม่ใช่ 'ทำการจอง') เลี่ยงคำเชื่อม AI ('อย่างไรก็ตาม','นอกจากนี้','อีกทั้ง') · เน้นคำ/ตัวเลขสำคัญด้วย <strong> ให้อ่านง่ายบนมือถือ · ห้ามคำลอย
...
ขั้นที่ 1 (คิดในใจ): ให้คะแนน 1-5 และหาจุดอ่อน — ย่อหน้าไหน generic/ลอย? จุดไหนควรมีตัวเลข/ราคา/ตัวอย่างแต่ยังกว้าง? content_gap/คำถามข้อไหนตอบไม่ครบ? ทุก H2/H3 คำถามมี answer-block 40-60 คำ quotable self-contained ไม่มีสรรพนามลอยครบไหม? มี E-E-A-T/information gain จริงไหม? ภาษาไทยตรงไหนแข็ง/แปลเครื่อง? ทุกตัวเลข/สถิติ/ราคา/ปี มาจากข้อมูลจริงในพิมพ์เขียว/ร่างหรือไม่ — ถ้าไม่ ต้องเปลี่ยนเป็น [ต้องเติม: ...] หรือช่วง+เงื่อนไข ('ขึ้นกับ...') ห้ามคงตัวเลขที่โมเดลเดาเอง
...
B2) รักษา/สร้างบล็อก <ul> 'ประเด็นสำคัญ' ใต้ <p> answer-first (3-5 bullet self-contained มีตัวเลข quotable) และแทรกประโยคนิยาม 'term คือ ...' ครบทุก term ใน definitions ที่จุดกล่าวถึง entity ครั้งแรก; ให้ทุก answer-block ขึ้นต้นด้วยการ echo คำสำคัญในหัวข้อ (H2 'X ราคาเท่าไหร่' → ย่อหน้าเริ่ม 'X ราคาอยู่ที่...') เพื่อให้ตรงกับ query ที่ผู้ใช้/AI ถาม
C) รีไรต์ภาษาไทยให้เป็นเจ้าของภาษา: อ่านออกเสียงทุกย่อหน้า ตรงไหนสะดุด/เหมือนแปลให้เขียนใหม่ · สลับความยาวประโยค (สั้น 6-12 คำ สลับยาว ห้ามยาวรวดเท่ากันทั้งย่อหน้า) · หนึ่งประโยคหนึ่งใจความ · เปลี่ยนคำนามธรรมเป็นกริยา ('ทำการจอง'→'จอง', 'ให้ความช่วยเหลือ'→'ช่วย') · ตัดคำฟุ่มเฟือย ('เป็นอย่างมาก','อย่างมีประสิทธิภาพ','ในการที่จะ','การที่','ซึ่งเป็น') · ตัดคำเชื่อม AI ซ้ำ ('อย่างไรก็ตาม','นอกจากนี้','อีกทั้ง','จะเห็นได้ว่า','ด้วยเหตุนี้') แทนด้วยคำที่คนไทยพูดจริงหรือขึ้นประโยคใหม่ · เลขอารบิก + ตัวคั่นหลักพัน — ตัวเลข/ความหมายเดิมห้ามเพี้ยน
D) เสริม E-E-A-T เนียน (ประสบการณ์จริง ข้อดี-ข้อเสีย) + CTA เนียน 2-3 จุด (ให้คุณค่าก่อนแล้วชวน) <a href> ไป {target_url} anchor ธรรมชาติ; internal link 2-4 จุด; เพิ่มสัญญาณความสด 1 จุดแบบเนียน (เช่น 'อัปเดตล่าสุดปี {year}') และเมื่ออ้างตัวเลข/ข้อเท็จจริงที่ต้องมีที่มา ให้ระบุแหล่งแบบสั้น (ไม่แต่งลิงก์ปลอม — ไม่มีจริงใส่ [ต้องเติม: แหล่งอ้างอิง])
...
<!--SCHEMA-->
(JSON-LD: ใช้ <script type="application/ld+json"> เพียงบล็อกเดียว บรรจุ JSON ออบเจ็กต์เดียวรูปแบบ {"@context":"https://schema.org","@graph":[...]} เท่านั้น — ห้ามหลาย object/หลาย <script> (ระบบ parse ทั้งก้อนครั้งเดียว ถ้าหลาย object จะพังแล้วถูกทิ้งทั้งหมด). ใน @graph ใส่: 1) Article หรือ BlogPosting: headline (<=110 อักษร ตรง title), description, inLanguage "th-TH", author และ publisher ({"@type":"Organization","name": ชื่อจากโดเมน {domain}}), datePublished และ dateModified (ISO ปี {year}), mainEntityOfPage, keywords (จาก target_keywords.entities), about ({"@type":"Thing","name": entity หลักจาก entity_glossary}); 2) FAQPage: ทุกคำถามใน 'คำถามที่พบบ่อย' เป็น Question -> acceptedAnswer {"@type":"Answer","text": คำตอบในบทความ 100% self-contained 40-60 คำ}; 3) HowTo: เพิ่มเฉพาะเมื่อบทความมี numbered_steps จริง — name + step[] แต่ละ {"@type":"HowToStep","name":...,"text":...} ตรงเนื้อ. ห้ามใส่ข้อมูล/ตัวเลขใน schema ที่ไม่มีในบทความ)
<!--NOTES-->
...
กติกา:
- แก้ตามจุดอ่อนข้างบนให้ครบ: ถ้าขาด answer-first ให้เปิดด้วยย่อหน้าตอบตรง 40-60 คำ; ถ้าขาด 'คำถามที่พบบ่อย' ให้เพิ่ม H2 คำถามที่พบบ่อย 4-8 ข้อพร้อมคำตอบ; ถ้าขาดนิยามให้เพิ่มประโยค 'X คือ...'; ถ้าขาดบล็อกสรุปประเด็นสำคัญ ให้เพิ่ม <p><strong>ประเด็นสำคัญ</strong></p><ul> 3-5 bullet self-contained มีตัวเลข ใต้ย่อหน้าแรก; ถ้าตื้นให้เพิ่มความลึก (ตัวเลข/ราคา/ตัวอย่าง/ขั้นตอน) ห้ามน้ำ; ถ้าขาด list/ตารางให้ใส่เมื่อเหมาะ; วางคีย์เวิร์ดหลักในย่อหน้าแรกและ H2 อย่างเป็นธรรมชาติ
...
- ภาษาไทยเจ้าของภาษา: เลี่ยงกลิ่นแปลเครื่องและคำเชื่อม AI ('อย่างไรก็ตาม','นอกจากนี้','อีกทั้ง','จะเห็นได้ว่า'), ใช้กริยาแทนคำนามธรรม ('จอง' ไม่ใช่ 'ทำการจอง'), สลับประโยคสั้น-ยาว, เลขอารบิก+ตัวคั่นหลักพัน — ห้ามเปลี่ยนตัวเลข/ข้อเท็จจริงเดิม

ส่ง 2 บล็อกตามลำดับเป๊ะ:
<!--ARTICLE-->
(HTML ที่ซ่อมแล้ว เริ่ม <p> answer-first หรือ <h2>)
<!--SCHEMA-->
(JSON-LD: <script type="application/ld+json"> บล็อกเดียว = JSON ออบเจ็กต์เดียว {"@context":"https://schema.org","@graph":[...]} ห้ามหลาย object. ใส่ Article/BlogPosting (headline, description, inLanguage "th-TH", author+publisher Organization, datePublished+dateModified ISO {year}, keywords, about Thing) + FAQPage (ทุกคำถามใน 'คำถามที่พบบ่อย', acceptedAnswer.text ตรงเนื้อ 100% self-contained 40-60 คำ) + HowTo เฉพาะเมื่อบทความมีขั้นตอน (step[] HowToStep). ห้ามแต่งข้อมูลเกินบทความ)"""
